...
with st.expander("ℹ️ なぜ「製品Critical = 0」でも「部材Critical」が存在するのか？", expanded=False):
    st.markdown(
        """
**この2つは独立した観点で評価されています。**

| 観点 | 評価対象 | 何を基準にCriticalと判定? |
|---|---|---|
| 📌 **製品の納期コミット** | 受注済みの**製品** | 顧客の希望納期まで**残3日以内** |
| 🎯 **半導体部材の調達** | 営業FCSTから展開した**部材** | 4ルート (顧客在庫/フリー/PO/新規発注) で需要を**満たせない**部材 |

**起こり得るパターン**:
- 受注がまだ確定していない先の月のFCSTで「部材不足」が予測される → 部材Criticalあり、製品Criticalなし
- 逆: 受注は確定済みだが部材は確保済 → 製品Criticalあり、部材Criticalなし

**結論**: 受注ベース (製品) と先読みの調達 (部材) は別の問題なので、両方の Critical を監視する必要があります。
"""
    )
