...
with st.expander("🤖 SCM Genie — 画面の数字の意味を確認・深掘り", expanded=False):
    ...
    if not genie.is_available:
        ...
        def _ask_genie(question: str):
            ...
            # st.status は内部的に expander のため、外側 expander 内では使えない。
            # プレーンな spinner + caption で進捗を見せる。
            progress_slot = st.empty()
            progress_slot.info("🤖 Genie に問い合わせ中... (LLM 推論 + SQL 実行)")
            try:
        ...
        with st.form("genie_freeform", clear_on_submit=True):
            prompt = st.text_input(
                "💬 質問入力",
                placeholder="例: 名古屋倉庫で安全在庫を割っている部品は?",
