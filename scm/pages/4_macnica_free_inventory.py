...
# 利用ガイドを上部 Expander に配置 (Phase 9)
with st.expander("📖 マクニカフリー在庫とは？（クリックで展開）", expanded=False):
    st.markdown(
        """
**マクニカフリー在庫**とは、マクニカが貴社向けに**事前に引当済みの在庫**です。

| | 顧客在庫 | マクニカフリー在庫 |
|---|---|---|
| 保管場所 | 貴社自社倉庫 | マクニカ新子安ロジセンター |
| 利用方法 | そのまま使用 | マクニカ営業に相談して出荷依頼 |
| リードタイム | 即時 | 輸送日数のみ (通常2〜3日) |
| 特徴 | 自社管理在庫 | 通常の調達LTを待たずに利用可能 |
...
**手順**: 「🚨 緊急調達シミュレーター」で評価 → ② フリー在庫ルートを確認 → マクニカ営業に相談用テキストを送付
"""
    )
...
# ────────────────────────────────────────────────────────
# 利用ガイドは上部 Expander に移動済み (Phase 9)
