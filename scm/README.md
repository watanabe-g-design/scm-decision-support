...
│   └── 03_genie_space_tuning.py    # Genie Space の Instructions/Sample queries/コメント設定
│
...
10. Databricks: notebooks/03_genie_space_tuning を Run All
    a. セル ① の Instructions 出力をコピー → Genie Space → Settings → Instructions に貼付
    b. セル ② の Sample queries 12 個を Genie Space → Sample queries に Add
    c. セル ③ で Catalog にカラムコメントが自動付与される
    → Genie の応答精度が劇的に向上 (この工程をスキップすると Genie が逆質問するだけになる)
11. (初回のみ) App のサービスプリンシパルに以下を手動付与:
...
### Databricks 不要ファイル / フォルダ
- `data_generation/` — デモデータ生成用。Databricksでは不要
- `sample_data/` — Pipeline の元データ。00_setup_catalog で Volume にコピー後は使われない

### 廃止された旧コード
- 旧 `logic/gold_builder.py` `safety_stock.py` `scoring.py` `glossary.py` は廃止しました
- デモモードも廃止しました (Databricks 接続必須)
- これらは Lakeflow Pipeline が作る Gold テーブルに完全に置き換わっています
